...
from ImageAbstract_Demo.code.artemis_model.in_out.basics import read_saved_args
from .datasets import AffectiveCaptionDataset, AbstrstWordsDataset
from ImageAbstract_Demo.code.artemis_model.neural_models.show_attend_tell import describe_model
...
def df_to_pytorch_dataset(df, args):
    ...
    datasets = pass_splits_to_datasets(df, load_imgs, img_transforms, args.img_dir, one_hot_emo=one_hot_emo)
...
def pass_splits_to_datasets(df, load_imgs, img_transforms, top_img_dir, one_hot_emo=True):
    datasets = dict()
    for split, g in df.groupby('split'):
        ...
        dataset = AbstrstWordsDataset(img_files,g.emotion_label,g.abstract_words_feature, g.tokens_encoded ,g.painting,
...
def image_emotion_distribution_df_to_pytorch_dataset(df, args, drop_thres=None):
    ...
    for split, g in df.groupby('split'):
        ...
        dataset = AbstrstWordsDataset(img_files, g.emotion_distribution,g.abstract_features,
...
def load_saved_speaker(args_file, model_ckp, with_data=False, override_args=None, verbose=False):
    ...
    args.file_name = 'artemis_preprocessed_enhance.csv'
    # Prepare empty model

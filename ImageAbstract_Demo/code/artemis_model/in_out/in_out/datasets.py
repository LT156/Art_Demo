...
class AffectiveCaptionDataset(Dataset):
    ...
    """
    def __init__(self,image_files, tokens, emotions,img_emo_id, emotion_histogram, n_emotions=9, img_transform=None, one_hot_emo=True):
        super(AffectiveCaptionDataset, self).__init__()
        self.unique_id = img_emo_id
        self.image_files = image_files
        self.tokens = tokens
        self.emotions = emotions
        self.emotion_histogram = emotion_histogram
    ...
    def __getitem__(self, index):
        ...
        emotion_histogram = np.array(self.emotion_histogram[index]).astype(dtype=np.float32)
        res = {'unique_id':unique_id,'image': img,'emotion_index' :self.emotions[index] , 'emotion': emotion,'emotion_histogram': emotion_histogram, 'tokens': text, 'index': index}
        return res
...
def group_annotations_per_image(affective_dataset):
    ...
    """
    df = pd.concat([affective_dataset.image_files, affective_dataset.tokens, affective_dataset.emotions,affective_dataset.emotion_histogram,affective_dataset.unique_id], axis=1)
    ...
    emotion_histogram_grouped = df.groupby('image_files')['emotion_histogram'].apply(lambda x:list(x)[0]).reset_index(name='emotion_histogram')
    emotion_gt_max=[]
    for e_list in emotion_grouped['emotion']:
        counts=Counter(e_list)
        emotion_gt_max.append(max(counts.keys(), key=counts.get))
    
    unique_id_grouped = df.groupby('image_files')['image_files'].apply(lambda x:x).reset_index(name='unique_id')
    
    assert all(tokens_grouped['image_files'] ==  emotion_grouped['image_files'])
    return tokens_grouped['image_files'], tokens_grouped['tokens_encoded'], emotion_gt_max,unique_id_grouped['unique_id'],emotion_histogram_grouped['emotion_histogram']

def group_annotations_per_image_by_unique_id(affective_dataset):
    """ Group the annotations per image.
    :param affective_dataset: an AffectiveCaptionDataset
    :return: for each image its tokens/emotions as pandas Dataframes
    """
    df = pd.concat([affective_dataset.image_files, affective_dataset.tokens, affective_dataset.emotions,affective_dataset.unique_id], axis=1)
    tokens_grouped = df.groupby('unique_id')['tokens_encoded'].apply(list).reset_index(name='tokens_encoded')
    emotion_grouped = df.groupby('unique_id')['emotion_label'].apply(lambda x:list(x)[0]).reset_index(name='emotion')
    unique_id_grouped = df.groupby('unique_id')['unique_id'].apply(lambda x:x).reset_index(name='unique_id')
    
    assert all(tokens_grouped['unique_id'] ==  emotion_grouped['unique_id'])
    return tokens_grouped['unique_id'], tokens_grouped['tokens_encoded'], emotion_grouped['emotion'],unique_id_grouped['unique_id']


def default_grounding_dataset_from_affective_loader(model, loader, img2emo_clf=None, device=None, n_workers=None):
    """
    ...
    img_files, tokens, emotions,unique_id,emotion_histogram = group_annotations_per_image(affective_dataset)
    ...
    if img2emo_clf is not None:
        if img2emo_clf=='original':
            grounding_emo = emotions
        elif img2emo_clf=='wscnet':
            ...
            emo_pred_distribution = wscnet_image_to_emotion( model.emotion_decoder, img_dataloader, device)
        else:
            temp_dataset = ImageClassificationDataset(image_files=img_files,
                                                    img_transform=img_trans)
            img_dataloader = DataLoader(temp_dataset, batch_size, num_workers=n_workers)
    ...
    new_dataset = AffectiveCaptionDataset(img_files, tokens=dummy, emotions=grounding_emo,img_emo_id=unique_id, emotion_histogram=emotion_histogram,
                                          img_transform=img_trans)

    new_loader = DataLoader(dataset=new_dataset, batch_size=batch_size, num_workers=n_workers)
    return new_loader

def default_grounding_dataset_from_affective_loader_by_unique_id(loader, img2emo_clf=None, device=None, n_workers=None):
    """
    Convenience function. Given a loader carrying an affective dataset, make a new loader only w.r.t.
    unique images of the dataset, & optionally add to each image the emotion predicted by the img2emo_clf.
    The new loader can be used to sample utterances over the unique images.
    ...
    """
    affective_dataset = loader.dataset
    img_files, tokens, emotions,unique_id = group_annotations_per_image_by_unique_id(affective_dataset)
    ...
    new_dataset = AffectiveCaptionDataset(img_files, tokens=dummy, emotions=grounding_emo,img_emo_id=unique_id,
                                          img_transform=img_trans)
...
def val_grounding_dataset_from_affective_loader(loader, img2emo_clf=None, device=None, n_workers=None):
    """
    Convenience function. Given a loader carrying an affective dataset, make a new loader only w.r.t.
    unique images of the dataset, & optionally add to each image the emotion predicted by the img2emo_clf.
    The new loader can be used to sample utterances over the unique images.
    ...
    """
    affective_dataset = loader.dataset
    img_files, tokens, emotions,unique_id = group_annotations_per_image(affective_dataset)
    ...
    new_dataset = AffectiveCaptionDataset(img_files, tokens=tokens, emotions=grounding_emo,img_emo_id=unique_id,
                                          img_transform=img_trans)

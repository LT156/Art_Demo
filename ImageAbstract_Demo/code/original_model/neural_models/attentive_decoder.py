...
class AttentiveDecoder(nn.Module):
    ...
    def __init__(self, word_embedding, rnn_hidden_dim, encoder_dim, attention_dim,
        ...
        self.bad_endings_ix=[int(k) for k,v in self.vocab.idx2word.items() if v in bad_endings]
        ...
        self.emo_f_beta = nn.Linear(auxiliary_dim, encoder_dim)  # linear layer to create a sigmoid-activated gate
        self.emol_f_beta = nn.Linear(rnn_hidden_dim, encoder_dim)  # linear layer to create a sigmoid-activated gate
    ...
    # img_feat, feature_map_split[emotion], caps_token, emotion
    ...
    #it,h,c, k_prev_words,encoder_out, aux_data
    def attend_and_predict_next_word(self, encoder_out, h, c, k_prev_words, aux_data=None,temperature=0.3):
        attention_weighted_encoding, alpha = self.attention(encoder_out, h)
        gate = self.sigmoid(self.f_beta(h))  # gating scalar, (batch_size_t, encoder_dim)
        attention_weighted_encoding = gate * attention_weighted_encoding
        embeddings = self.word_embedding(k_prev_words)  # (batch_size, embed_dim)
        ...
        logits = self.next_word(h)  

        if temperature != 1:
            logits /= temperature
        return h, c, logits, alpha,F.log_softmax(logits, dim=1)
...
def single_epoch_train(train_loader, model, criterion, optimizer, epoch, device,  tb_writer=None, **kwargs):
    ...
    for i, batch in enumerate(train_loader):
        ...
        #images,tokens,captions,auxiliary_data)
        if use_emotion:
            emotion = batch['emotion'].to(device)
            emotion_index = batch['emotion_index'].to(device)
            emotion_histogram = batch['emotion_histogram'].to(device)
            res = model(mode, imgs, emotion)
        else:
            res = model.decoder(model.encoder(imgs), caps_token)
...
def negative_log_likelihood(model, data_loader, device,args):
    ...
    for batch in data_loader:
        imgs = batch['image'].to(device)
        caps_token = batch['tokens'].to(device)
        img_feat=model.encoder(imgs)
        

        # TODO Refactor
        #if model.decoder.attentive_decoder.uses_aux_data:
        if model.decoder.uses_aux_data:
            emotion = batch['emotion'].to(device)
            emotion_index = batch['emotion_index'].to(device)
            emotion_histogram = batch['emotion_histogram'].to(device)
            res = model(args.mode, imgs, emotion)
        else:
            res = model.decoder(img_feat, caps_token)
        ...
        emotion_histogram = torch.softmax(emotion_histogram, dim=-1)
...
def sample_captions_beam_search(model, data_loader, beam_size, device, temperature=1, max_iter=500,
    ...
    model.eval()
    '''
    decoder = model.decoder.attentive_decoder
    vocab = model.decoder.attentive_decoder.vocab
    '''
    decoder = model.decoder
    ...
    for batch in tqdm.tqdm(data_loader):  # For each image (batch-size = 1)
        ...
        if model.decoder.uses_aux_data:
            ...
            emotion = batch['emotion'].to(device)
            aux_data=emotion
            #aux_feat = model.decoder.emo_projection_net(aux_data)
        ...
        encoder_dim = 512
        encoder_out=encoder_out.view(len(image),-1,512) 

...
def pickle_data(file_name, *args):
    ...
    pickle.dump(len(args), out_file, protocol=2)
    for item in args:
        pickle.dump(item, out_file, protocol=2)
    out_file.close()
...
def unpickle_data(file_name, python2_to_3=False):
    ...
    if python2_to_3:
        size = pickle.load(in_file, encoding='latin1')
    else:
        size = pickle.load(in_file)

    for _ in range(size):
        if python2_to_3:
            yield pickle.load(in_file, encoding='latin1')
        else:

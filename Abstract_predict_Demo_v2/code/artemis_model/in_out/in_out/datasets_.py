...
from artemis.evaluation.emotion_alignment import image_to_emotion
...
class AffectiveCaptionDataset(Dataset):
    ...
    """
    def __init__(self, img_files, data, n_emotions=9, n_background=512,img_transform=None, one_hot_emo=True,use_background=False):
        super(AffectiveCaptionDataset, self).__init__()
        self.unique_id = data.painting
        self.image_files = img_files
        self.tokens = data.tokens_encoded
        self.emotions = data.emotion_label
    ...
    def __getitem__(self, index):
        ...
        if self.use_background:
            background_tokens = np.array(self.background[index]).astype(dtype=np.long)
        else:
            background_tokens=[]
        
        res = {'unique_id':unique_id,'image': img, 'emotion': emotion, 'tokens': text, 'background': background_tokens,'index': index}
        return res
...
def group_annotations_per_image(affective_dataset,use_background):
    ...
    """
    df = pd.concat([affective_dataset.image_files,affective_dataset.unique_id, affective_dataset.tokens, affective_dataset.emotions], axis=1)
    painting_grouped = df.groupby('image_files')['painting'].apply(list).reset_index(name='painting')
    if use_background:
        df['background'] = affective_dataset.background
    ...
    if use_background:
        background_grouped = df.groupby('image_files')['background'].apply(list).reset_index(name='background')
        background_data=background_grouped['background']
    else:
        background_data=None
    
    assert all(tokens_grouped['image_files'] ==  emotion_grouped['image_files'])
    return painting_grouped['painting'],tokens_grouped['image_files'], tokens_grouped, emotion_grouped,background_data


def default_grounding_dataset_from_affective_loader(loader, img2emo_clf=None, device=None, n_workers=None,use_background=False):
    """
    ...
    paintings,img_files, tokens, emotions,background = group_annotations_per_image(affective_dataset,use_background)
    ...
    '''
    result=pd.DataFrame()
    result["tokens_encoded"]=dummy
    result["emotion_label"]=grounding_emo
    result["painting"]=paintings
    if use_background:
        result["background"]=background

    new_dataset = AffectiveCaptionDataset(img_files,data=result,img_transform=img_trans,use_background=use_background)

...
from Xmodal_Ctx_lcs.neural_models_v1.Caption_model import Caption_model 

from ast import literal_eval
from PIL import Image

from Xmodal_Ctx_lcs.in_out.basics import read_saved_args
from Xmodal_Ctx_lcs.in_out.datasets import AffectiveCaptionDataset,ImageClassificationDataset
from artemis.utils.vocabulary import Vocabulary
from artemis.neural_models.show_attend_tell import describe_model as describe_sat
...
def df_to_pytorch_dataset(df, args):
    ...
    if args.dataset == 'artemis':
        datasets = pass_artemis_splits_to_datasets(df, load_imgs, img_transforms, args.img_dir, one_hot_emo=one_hot_emo,use_background=args.use_background)
...
def pass_artemis_splits_to_datasets(df, load_imgs, img_transforms, top_img_dir, n_emotions=9, one_hot_emo=True,use_background=False):
    datasets = dict()
    for split, g in df.groupby('split'):
        g.reset_index(inplace=True, drop=True) # so that direct ([]) indexing in get_item works
        img_files = None
        ...
        dataset = AffectiveCaptionDataset(img_files, g, n_emotions=n_emotions,
                                          img_transform=img_trans, one_hot_emo=one_hot_emo,use_background=use_background)
...
def load_saved_speaker(args_file, model_ckp, with_data=False, override_args=None, verbose=False):
    ...
    model = Caption_model(vocab=vocab, args=args)
    ...
    if with_data:
        ...
        if args.use_background:
            back_df=pd.read_csv(args.background_dir)
            merged=pd.merge(df,back_df,on='painting',how='inner')
            assert len(merged)==len(df),'df length is too long '
            df['background']=merged['content_encoded']
            df.background = df.background.apply(literal_eval)
            print('Will use {} background annotations for training.'.format(len(merged)))
...
##################################################mycode###################################################
def read_preprocessed_data_df_mycode(args, vocab,verbose=False):
    if args.dataset == 'artemis':
        file_name = 'artemis_preprocessed.csv'
    elif args.dataset == 'coco':
        file_name = 'coco_preprocessed.csv'
    ...
    ###追加辅助信息
    if args.use_ann_csv:
        ann_df=pd.read_csv(args.annotation_csv)
        df_tmp=pd.merge(df,ann_df,how='inner',on='semArt_url')
        df['obj_labels']=df_tmp['obj_labels']
        df['obj_labels_encoded']=df_tmp['obj_labels_encoded']
        df['obj_boses']=df_tmp['obj_boses']
    def padding_labels(np_array,max_dim=13):
        np_array=np.lib.pad(np_array, (0,max_dim-len(np_array)), 'constant', constant_values=(0))
        return np_array
    df.obj_labels_encoded = df.obj_labels_encoded.apply(literal_eval)
    df.obj_labels_encoded = df.obj_labels_encoded.apply(padding_labels)
    
    if verbose:
        print('Loaded {} utterances'.format(len(df)))
    return df

def pass_artemis_splits_to_datasets_mycode(df, load_imgs, img_transforms, top_img_dir,n_obj_labels, n_emotions=9, one_hot_emo=True):
    datasets = dict()
    for split, g in df.groupby('split'):
        g.reset_index(inplace=True, drop=True) # so that direct ([]) indexing in get_item works
        img_files = None
        ...
        if load_imgs:
            ...
            img_files.name = 'image_files'
            img_trans = img_transforms[split]
        # image_files, tokens, emotions,obj_labels,n_emotions=9,n_obj_labels=None, img_transform=None, one_hot_emo=True)
        dataset = AffectiveCaptionDataset(img_files, g.tokens_encoded, g.emotion_label, g.obj_labels_encoded,n_emotions=n_emotions,n_obj_labels=n_obj_labels,
                                          img_transform=img_trans, one_hot_emo=one_hot_emo)

        datasets[split] = dataset
    return datasets

def df_to_pytorch_dataset_mycode(df, args):
    ...
    if args.dataset == 'artemis':
        if args.use_ann_csv==False:
            datasets = pass_artemis_splits_to_datasets(df, load_imgs, img_transforms, args.img_dir, one_hot_emo=one_hot_emo,use_background=use_background)
        else:
            datasets = pass_artemis_splits_to_datasets_mycode(df, load_imgs, img_transforms, args.img_dir, n_obj_labels=args.annotation_dim,one_hot_emo=one_hot_emo)
    elif args.dataset == 'ola': # Objective Language for Art.
        datasets = pass_artemis_splits_to_datasets(df, load_imgs, img_transforms, args.img_dir, n_emotions=0)
    elif args.dataset == 'coco':
        datasets = pass_coco_splits_to_datasets(df, load_imgs, img_transforms)
...
def load_saved_speaker_mycode(args_file, model_ckp, with_data=False, override_args=None, verbose=False):
    """
    :param args_file: saved argparse arguments with model's description (and location of used data)
    :param model_ckp: saved checkpoint with model's parameters.
    :param with_data:
    ...
    vocab = Vocabulary.load(osp.join(args.data_dir, 'vocabulary.pkl'))
    print('Using a vocabulary of size', len(vocab))
    model = describe_sat_mycode(vocab, args)
    ...
    # Load data
    if with_data:
        df = read_preprocessed_data_df_mycode(args, vocab, verbose=True)
        data_loaders, _ = df_to_pytorch_dataset_mycode(df, args)
    else:
        data_loaders = None

    return model, epoch, data_loaders,args

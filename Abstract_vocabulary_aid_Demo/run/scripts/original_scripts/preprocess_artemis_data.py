...
#
freq_file = osp.join(up_dir, '/media/data/LuoTing/local/work_space/artemis-master/artemis/data/symspell_frequency_dictionary_en_82_765.txt')
glove_file = osp.join(up_dir, '/media/data/LuoTing/local/work_space/artemis-master/artemis/data/glove.6B.100d.vocabulary.txt')
...
def parse_arguments(notebook_options=None):
    ...
    flag=False
    # Required arguments
    parser.add_argument('-save-out-dir', type=str, required=flag, help='where to save the processed data')
    parser.add_argument('-raw-artemis-data-csv', type=str, required=flag, help='dataset (csv file) provided upon YOUR request!')
    ...
    args.save_out_dir='/media/data/LuoTing/local/work_space/out/raw/quarter_artemis'
    args.raw_artemis_data_csv='/media/data/LuoTing/local/work_space/out/raw/quarter_artemis.csv'
    args.preprocess_for_deep_nets=True
...
def group_gt_annotations(preprocessed_dataframe, vocab):
    ...
    for split, g in df.groupby('split'): # group-by split
        ...
        g = g.groupby(['painting']) # group-by stimulus
        ...
        #assert all(tokens_grouped['img_emo_id'] == refs_pre_vocab_grouped['img_emo_id'])
        ...
        for idx,row in result.iterrows():
            result_dict[row['painting']]=row['references']
        reference_dict[split]=result_dict
        results[split] = result

    return results,reference_dict
...
def preprocess(args, verbose=True, test_masterpieces=False):
    ...
    if args.too_high_repetition != -1:
        ...
        high_coverage_df = high_coverage_df.assign(split=len(high_coverage_df)*['train'])
...
def reduced_repetition(df):
    repetition_mean=int(np.array(df['repetition']).mean())

    g_list=[]
    for _,g in df.groupby('painting'):
        g.reset_index(inplace=True, drop=True)
        repetition = g['repetition'][0]
        if repetition>repetition_mean:
            #随机抽
            #new_index=random.sample(range(0,repetition),repetition_mean)
            g=g.iloc[:repetition_mean]
        g_list.append(g)
    final_df=pd.concat(g_list, ignore_index=True)
    return final_df
if __name__ == '__main__':
    ...
    df['scent_id']=list(range(len(df)))
    df['img_emo_id']=df['painting']+df['emotion']
    ...
    if args.group_gt_anno:
        groups_split,reference_dict = group_gt_annotations(df, vocab)
        json_data = json.dumps(reference_dict)
        with open(osp.join(args.save_out_dir, 'artemis_gt_encoded.json'), 'w') as f_six:
            f_six.write(json_data)
        pickle_data(osp.join(args.save_out_dir, 'artemis_gt_references_grouped.pkl'), groups_split)

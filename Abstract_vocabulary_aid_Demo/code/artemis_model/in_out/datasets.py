...
class AffectiveCaptionDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        abstract_data = np.array(self.abstract_words_feature[index]).astype(dtype=np.float32)
...
def group_annotations_per_image(affective_dataset):
    ...
    """
    df = pd.concat([affective_dataset.image_files, affective_dataset.tokens, affective_dataset.emotions,affective_dataset.abstract_words_feature,affective_dataset.unique_id], axis=1)
    ...
    abstract_words_feature =  df.groupby('image_files')['abstract_features'].apply(lambda x:np.sum(x,axis=0)).reset_index(name='abstract_features')
    ...
    return tokens_grouped['image_files'], tokens_grouped['tokens_encoded'], emotion_gt_max,abstract_words_feature['abstract_features'],unique_id_grouped['unique_id']
...
def default_grounding_dataset_from_affective_loader(model, loader, img2emo_clf=None, device=None, n_workers=None):
    ...
    img_files, tokens, emotions,abstract_words_feature, unique_id = group_annotations_per_image(affective_dataset)
    ...
    new_dataset = AffectiveCaptionDataset(img_files, abstract_words_feature, tokens=dummy, emotions=grounding_emo,img_emo_id=unique_id, 
                                          img_transform=img_trans)

...
class AttentiveDecoder(nn.Module):
    ...
    def __init__(self, word_embedding, rnn_hidden_dim, encoder_dim, attention_dim,
                 ...
                 auxiliary_net=None, abstract_projection_net=None,auxiliary_dim=0,abstract_dim=0):
        """
        ...
        self.bad_endings_ix=[int(k) for k,v in vocab.idx2word.items() if v in bad_endings]
        self.vocab = vocab
        ...
        self.uses_abstract_data = False

        if auxiliary_dim > 0:
            self.uses_aux_data = True
        if abstract_dim>0:
            self.uses_abstract_data =True

        self.decode_step = nn.LSTMCell(word_embedding.embedding_dim + encoder_dim+abstract_dim + auxiliary_dim, rnn_hidden_dim)
        ...
        self.init_h = nn.Linear(encoder_dim+abstract_dim+auxiliary_dim, rnn_hidden_dim)  # linear layer to find initial hidden state of LSTMCell
        self.init_c = nn.Linear(encoder_dim+abstract_dim+auxiliary_dim, rnn_hidden_dim)  # linear layer to find initial cell state of LSTMCell
        self.f_beta = nn.Linear(rnn_hidden_dim, encoder_dim)  # linear layer to create a sigmoid-activated gate
        self.abstract_f_beta = nn.Linear(rnn_hidden_dim, abstract_dim)  # linear layer to create a sigmoid-activated gate
    ...
    def __call__(self, encoder_out, captions, auxiliary_data=None,abstract_data=None):
        """ Forward propagation.
        ...
        """
        return self.sort_captions_and_forward(encoder_out, captions, auxiliary_data=auxiliary_data,abstract_data=abstract_data)

    def sort_captions_and_forward(self, encoder_out, captions, auxiliary_data=None,abstract_data=None):
        """ Feed forward that ...
        :param encoder_out:
        :param captions:
        :return:
        ...
        for t in range(max(decode_lengths)):
            ...
            if abstract_data is not None:
                abstract_data_t = abstract_data[:batch_size_t]
                # gate
                gate = self.sigmoid(self.abstract_f_beta(h))  # gating scalar, (batch_size_t, encoder_dim)
                abstract_data_t = gate * abstract_data_t
                decoder_in = torch.cat([decoder_in, abstract_data_t], dim=1)
...
def single_epoch_train(train_loader, model, criterion, optimizer, epoch, device, tb_writer=None, **kwargs):
    ...
    for i, batch in enumerate(train_loader):
        ...
        # Calculate loss
        ent_loss = criterion(logits.data, targets.data.long())
        total_loss = ent_loss
...
def negative_log_likelihood(model, data_loader, device):
    ...
    for batch in data_loader:
        ...
        # Calculate loss
        loss = F.cross_entropy(logits.data, targets.data.long())
...
def sample_captions_beam_search(model, data_loader, beam_size, device, temperature=1, max_iter=500,
    ...
    for batch in tqdm.tqdm(data_loader):  # For each image (batch-size = 1)
        ...
        while True:
            ...
            prev_word_inds = torch.div(top_k_words, len(vocab), rounding_mode='floor')#top_k_words // len(vocab)  # (s)

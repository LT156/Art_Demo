...
class Abs_Attr_Model(nn.Module):
    ...
    def forward(self, img_feats, obj_feat,  attr_feats):
        ...
        focal_loss,attr_index = self.multimodal_detector(visual_feat, attr_feats, self.selected_num)
...
def single_epoch_train(model, data_loader, optimizer, device):
    ...
    for batch in tqdm.tqdm(data_loader):
        ...
        focal_loss, _ = model(img_feat,obj_feat,abs_feat)
...
def single_epoch_eval(model, data_loader, device,args):
    ...
    for batch in tqdm.tqdm(data_loader):
        ...
        obj_feat = batch['obj_feat'].to(device)
        abs_feat = batch['abstract_data'].to(device)
        focal_loss, attr_index = model(img_feat,obj_feat,abs_feat)
...
def evaluation_index(model, data_loader, device,args):
    ...
    for batch in tqdm.tqdm(data_loader):
        ...
        obj_feat = batch['obj_feat'].to(device)
        abs_feat = batch['abstract_data'].to(device)
        _, attr_index = model(img_feat,obj_feat,abs_feat)
    ...
    return precision_sum.avg,recall_sum.avg

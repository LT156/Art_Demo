...
def pass_artemis_splits_to_datasets(df, load_imgs, img_transforms, top_img_dir, one_hot_emo=True):
    datasets = dict()
    for split, g in df.groupby('split'):
        g.reset_index(inplace=True, drop=True) # so that direct ([]) indexing in get_item works
        img_files = None
        ...
        if load_imgs:
            ...
            img_trans = img_transforms[split]
        dataset = AffectiveCaptionDataset(img_files, g.tokens_encoded, g.emotion_label,g.unique_id, g.emotion_histogram,
...
def load_saved_speaker(args_file, model_ckp, with_data=False, override_args=None, verbose=False):
    ...
    if with_data:
        ...
        import pandas as pd
        emotion_histogram = pd.read_csv(args.emotion_histogram)
        from ast import literal_eval
        emotion_histogram['emotion_histogram'] = emotion_histogram['emotion_histogram'].apply(literal_eval)
        emotion_histogram = emotion_histogram[['painting','emotion_histogram']]
        df = pd.merge(df, emotion_histogram, on='painting', how='inner')

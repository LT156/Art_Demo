...
import pandas as pd
import numpy as np
from Image_affective_recognition_Demo_v2.code.utils.config import CfgNode
from Image_affective_recognition_Demo_v2.code.artemis_model.in_out.neural_net_oriented import  torch_save_model
from Image_affective_recognition_Demo_v2.code.artemis_model.in_out.basics import create_dir
from Image_affective_recognition_Demo_v2.code.model.Image_Emotion_Clf import ImageEmotionClassifier, evaluation_index, single_epoch_eval, single_epoch_train
from Image_affective_recognition_Demo_v2.code.in_out.dataloader_to_pack import dataloader_pack
...
def init_model(args):
    ...
    with open(args.vocab_path, 'rb') as f:
        vocab = pickle.load(f)
    model = ImageEmotionClassifier(args,len(vocab))
    return  model
...
def train(model, data_loaders, optimizer, device,args):
    ...
    for epoch in range(1, args.max_train_epochs+1):
        ...
        eval_loss,precision,recall,acc = single_epoch_eval(model, data_loaders['val'], device, args)
        print('Epoch {} Eval Loss: {:.6f}\t Eval_Precision:{:.4f}\t Eval_Recall: {:.4f}\t Eval_Acc: {:.4f} '.format(epoch,eval_loss,precision,recall,acc))

        if eval_loss < min_eval_loss:
            min_eval_loss = eval_loss
            no_improvement = 0
            print('Epoch {}. Validation loss improved!'.format(epoch))
            torch_save_model(model, args.checkpoint_file+'/best_model.pkl')
       
            precision,recall,acc = evaluation_index(model, data_loaders['test'], device,args)
            print('Epoch {} Test_Precision:{:.4f}\t Test_Recall: {:.4f}\t Test_Acc: {:.4f} '.format(epoch,precision,recall,acc))
            best_model = model
        else:
            no_improvement += 1
...
def get_args():
    ...
    # config
    parser.add_argument('--cfg', type=str, default='F:/work/Image_emotion_analysis/artemis-master/Image_affective_recognition_Demo_v2/config/image2word_local.yml',
...
if __name__=='__main__':
    ...
    precision,recall,acc = evaluation_index(model, data_loaders['test'], device,args)
    print('Finially, Test_Precision:{:.4f}\t Test_Recall: {:.4f}\t Test_Acc: {:.4f} '.format(precision,recall,acc))
    print('END')
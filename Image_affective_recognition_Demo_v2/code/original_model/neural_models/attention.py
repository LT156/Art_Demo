...
class AdditiveVisioLinguistic(nn.Module):
    ...
    def __call__(self, encoder_out, decoder_hidden):
        """#encoder_out=k,decoder_hidden=q
        Forward propagation.
        ...
        att = self.full_att(self.relu(att1 + att2.unsqueeze(-2))).squeeze(-1)  # (batch_size, num_pixels)
        alpha = self.softmax(att)  # (batch_size, num_pixels)
        attention_weighted_encoding = (encoder_out * alpha.unsqueeze(-1)).sum(dim=-2)  # (batch_size, encoder_dim)
